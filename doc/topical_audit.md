...
- **PBC ribbons:** `ribbon_pbc.build_ribbon_cell(width_chains, ncells, passivation)` over `build_zigzag_ribbon(..., bPeriodicX=True)` — zigzag GNR periodic along x; edge passivation replaces edge C per `PASSIVATION_GROUPS` ('N' = pyridinic edge, 'NH' = protonated edge; per-site strings via `parse_passivation_string`: n→NH, N→N, o→C=O, O→O, H→CH, h→C-OH). Wrap bonds across the x seam are in `atoms.bonds` (+ `seam_mask` out). Driver: `tests/topology/testplot_ribbon.py` → `debug/ribbon/` (tiled-cell PNG + XYZ with lvs; `--dftb` runs `run_pbc` SP, nk along x)
