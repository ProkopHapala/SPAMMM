...
- **PAH flakes:** `tests/topology/testplot_mol_flakes.py` — symmetric fused-ring flakes from ring-centre row profiles (`profile_rings`), `hex_rings` (coronene), `tri_rings` (triangulene); non-convex grid fillers as ASCII arts via `build_art_flake` (biphenyl, pterphenyl, w2h5, w3h3 in `EXT`); skeleton via `parse_ascii_art` → `AtomicGraph` → per-site edge chars `C/c/N/n/O/o` (`O`=keto `C=O`, `o`=enol `C–OH`) → `adjust_h` caps; `check_mirror_sym` verifies x/y mirrors (both mirrors need odd-nrow parity-consistent palindromes — rectangles >1 row can never be x-symmetric). Overview = width×height matrix. → `debug/mol_flakes/` (PNG+XYZ+overview); study plan in `doc/ERC_private/task_Molecules.md`
- **PAH flake DFTB battery:** `tests/topology/testplot_mol_flakes_dftb.py` — 9 backbones × {C,N,O} two-site switch chemistries × {0H, 2H(+T), 1H*, 1H+, 1H−} = 162 fixed-skeleton DFTB3+spin SPs (`testplot_muH._patch_hsd`); caches `results.json`, `sp/`, `relax_bat/` + `results_relax.json`, `bo/*.npz`. Figures: `battery_report.png` (bars+thumbnails), `battery_lines.png` (0H–1H–2H ladders), `battery_reservoir.png` (Ω=E−q·μ_e, μ_e=−φ; φ_Au=5.3, φ_Gr=4.6 eV; μ_H=E(H₂)/2=−9.123 eV). Per-backbone bond maps (`--maps`): `bo_<name>.png` = π-BO from DFTBcore SP density (`pi_bond_order`: `ao_layout`/`pi_project`/`lowdin_orthogonalize`/`bond_orders_from_pmat` → `plot_bond_scalar_map`), `bl_<name>.png` = relaxed bond lengths; cols Aromatic|Quinonic|Aro−Qui|1H*|1H+|1H−, rows C/N/O — O flipped: enol=aromatic, keto=quinonic → `debug/mol_flakes_dftb/` (PNG+SVG)
...
- **PBC ribbons:** `ribbon_pbc.build_ribbon_cell(width_chains, ncells, passivation)` over `build_zigzag_ribbon(..., bPeriodicX=True)` — zigzag GNR periodic along x; edge passivation replaces edge C per `PASSIVATION_GROUPS` ('N' = pyridinic edge, 'NH' = protonated edge; per-site strings via `parse_passivation_string`: n→NH, N→N, o→C=O, O→O, H→CH, h→C-OH, m→CH2; groups CH, N, NH, O, C=O, C-OH, CH2, CHOH in `PASSIVATION_GROUPS`). Wrap bonds across the x seam are in `atoms.bonds` (+ `seam_mask` out). Driver: `tests/topology/testplot_ribbon.py` → `debug/ribbon/` (tiled-cell PNG + XYZ with lvs; `--dftb` runs `run_pbc` SP, nk along x)
- **Ribbon junction cells:** `ribbon_pbc.build_ribbon_junction_cell(width_chains, ncells, d_DA, state, pbc_y=True)` — stacks an acceptor ribbon (N edges) and a donor ribbon (NH edges) along y so every interface is an N···H-N junction (2 per cell with pbc_y: internal + boundary, `HbondRecord.a_shift`). `state` = 2·ncells chars over {p,d,0} (proton up/down/bare) overrides bottom/top per site; p↔d inversion is an exact mirror symmetry (parity-checked in scans). Auto-aligns staggered zigzag edges in x. Returns `(AtomicSystem, lvs, hbonds)` — the `build_pbc_cell` contract, so `coordinate_scan.run_corner_scan_pbc` works directly (mapping groups the `ncells` sites per interface into controls). Driver: `testplot_ribbon.py --two`
- **Junction gap scan:** `ribbon_pbc.scan_junction_gap(w, ncells, gaps, states, nk, relax)` — E(d_DA) lattice-y optimization via `run_pbc` (SP or `--relax`), exact parabola through the 3 lowest points (analytic, not least squares), p↔d parity report; `plotUtils.plot_gap_scan` draws it. Optimum (w4 n2, 3ob-3-1, nk 8×2): d_DA ≈ 3.0 Å (uniform 2.88 / alternating 3.02 relaxed); alternating ~0.9 eV below uniform; k-insensitive (nk8→32 Δd_opt ≤0.004 Å). See `doc/ERC_private/ribbon_junction_cells.md`.
- **Tests:** `tests/topology/test_editing_ops.py`, `tests/test_export_import.py`, `tests/topology/testplot_ribbon.py`
...
### 5a. Hydrogen chemical potentials & proton-transfer energetics (μH model)

- Local per-site H affinities `μ1=E(XH)−E(X)`, `μ2=E(XH₂)−E(XH)`, cooperativity `J=μ2−μ1`, closed-shell 2H affinity `μ2H=E(XH₂)−E(X)`; transfer energies predicted as μ differences so all element offsets cancel (no reservoir needed). Molecules: BQ, NQ, Pyr, QX, Phz (+Qnol lactam/lactim, charged XH⁻/XH⁺ for the ionic PT channel).
- **Driver:** `tests/topology/testplot_muH.py` (self-contained; caches `debug/muH/results.json`, idempotent `--skip-mols/--skip-dimers/--figonly`). **Report:** [ERC_private/muH_monomers.md](ERC_private/muH_monomers.md). **Artifacts:** `debug/muH/` — `mu_ladder.png`, `transfer_calib.png`, `transfer_2h_matrix.png`, `rxn_*.png`, `rxn2h_*.png`, `build_check.png`.
- **Protocol:** 3ob-3-1 **requires full DFTB3** (`ThirdOrderFull` + `HubbardDerivs` + `HCorrection=Damping{4.0}` — this build predates the `DampXH` name); odd-e states spin-polarized (`UnpairedElectrons`, mio SpinConstants — 3ob .skf ships none); dimer corners pinned `{D,A,H}`.
- **Results:** all monomer J<0 (−0.76…−1.19 eV, cooperative 2nd H); single-H dimers agree to ±0.2 eV (5/6), q+hq +0.59; ionic PT channel +5–7 eV uphill in gas phase; closed-shell 2H matrix — μ2H ordering NQ>BQ>Phz>QX>Pyr, PyrH₂ best donor (see report for reuse API).
- **Caveats:** `:` junction resolves by character column → verify `atoms.hbonds_ascii` element pairs (phz C–H···C false alarm); fragment Mulliken sums on H-bonded dimers are a partitioning artifact — audit charge transfer by dipole; `makeDFTBjob_pbc` still DFTB2/no-spin (chain J's pending re-check).
- **Related:** chain corner scans `testplot_corner_scan_pbc.py` + [ERC_private/pbc_proton_transfer_chains.md](ERC_private/pbc_proton_transfer_chains.md) (J's compared in report); ribbon junctions `testplot_ribbon.py` (§1f).

### 5b. Ribbon band structure & supercell unfolding

- Periodic zigzag ribbons with per-site edge passivation → DFTB+ bands → supercell→primitive spectral unfolding (does edge hydrogenation break the π channel / shift spectral weight?). **Full detail:** [TopicalAudit/BandUnfolding_Ribbons.md](TopicalAudit/BandUnfolding_Ribbons.md)
- **Driver:** `tests/quantum/testplot_unfold.py` — modes: canonical x1/x2/x4 folding overlay (default; replicas + extended-zone run + weight dots), `--passiv` x1 edge-chemistry comparison (C=g/N=b/O=r, ±4 eV, width in filename), `--switch` per-system ref-vs-hydrogenated (black/red), `--sys3` relaxed one-site-hydrogenated x2/x4 with unfolded weights, `--perturb/--rdistort` symmetry-breaking probes
- **Key files:** `spammm/quantum/pi_bond_order.py` (`subcell_group_indices`, `unfold_T_weights` — canonical S(k)-metric translation-projector weights; `unfold_spectral_weights_DEPRECATED` — Euclidean formula, wrong for nonorthogonal bases, `read_band_out`, `read_eigenvec_bin`), `spammm/quantum/DFTB_utils.py` (`run_pbc`/`makeDFTBjob_pbc`: `k_shift=(0,0,0)` Γ-mesh incl. BZ edge, `klist=` explicit kpts, `Mixer` caveat), `spammm/topology/MoleculeEditorBackend.py` (`PASSIVATION_GROUPS` incl. CH/N/NH/CH2/C=O/C-OH/CHOH), `spammm/topology/ribbon_pbc.py`
- **Parity:** fold-down 0.1 meV; extended-zone vs replicated 0.2 meV; pristine unfold 91.2% sharp (degeneracies only); gallery `debug/unfold/index.html`
- **Result highlights:** CH2 is the only on/off switch and only on even widths (gap 3.7→0.28 eV for w4→w12); NH, C=O are doping switches (HOMO shifts ~4–5 eV); all sp2 edges semimetallic
- **Derivation:** [chats/Band_Unfolging.chat.md](chats/Band_Unfolging.chat.md) (PRB 89, 201412 suppl.)
- **Caveats:** DIIS singular on Γ-mesh metals → Broyden; weights at exact degeneracies arbitrary; `--switch` O-pair is C-OH↔C=O vs `--sys3` C-OH↔CHOH — different chemistry; fold-check on perturbed cells measures band shifts, not validity; **never unfold with the Euclidean formula on S≠I bases** (DFTB mio |S_off|~0.44, GPAW dzp ~0.5 — leaked ~25% of weight, artificial |f_s|-dependent wash; `unfold_T_weights` restores 0H to 100% sharp)
