...
__kernel void evalAngles_UFF(
    ...
    if ((DBG_UFF>10) && iang==IDBG_ANGLE && (isys==IDBG_SYS)){
...
__kernel void evalDihedrals_UFF(
    ...
    int i0f = isys * nf_per_system; // per-system base for fint
    if ((DBG_UFF>1) && id==IDBG_DIH && (isys==IDBG_SYS)){
...
__kernel void evalInversions_UFF(
    ...
    int i0f = isys * nf_per_system;  // per-system base for fint
    if ((DBG_UFF>1) && ii==IDBG_INV && (isys==IDBG_SYS)){
...
__kernel void assembleForces_UFF(
    ...
    if ( (DBG_UFF>1) &&  (ia == 0) && (isys==IDBG_SYS) ){
...
__kernel void updateAtomsSPFFf4(
    ...
    if((DBG_UFF>1) && (iS==IDBG_SYS)&&(iG==IDBG_ATOM)){
        printf("GPU updateAtomsSPFFf4[isys=%i]: MDpars(dt=%g,Flim=%g,vel_damp_factor=%g,?= %g) natoms=%i nS=%i\n", iS, MDpars.x,MDpars.y,MDpars.z,MDpars.w, natoms, nS );
