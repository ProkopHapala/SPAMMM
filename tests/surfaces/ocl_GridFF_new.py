"""
ocl_GridFF_new.py — GridFF construction and sampling test utilities.

Purpose: Helper functions for building GridFF potentials and sampling them at
arbitrary points. Includes PLQH (Pauli, London, Coulomb, Hydrogen) parameter
extraction and 1D/2D potential plotting for verification.

Key functionality:
  - build_gridff_from_system() — construct GridFF from molecule + substrate
  - sample_gridff_at_points() — interpolate potential at test points
  - plot1Dcut() — visualize potential along a line
  - getPLQH() — extract PLQH parameters from ElementTypes.dat

Role in SPAMMM: GridFF testing and construction utilities. Used by tests and
tutorials to build and verify grid potentials. Also provides reference
potential computation for parity checks.
"""
...
def test_Ewald( apos, qs, ns=[100,100,100], dg=(0.1,0.1,0.1), nPBC=[30,30,30], pos0=None, scErr=100.0, order=2, bPython=True, bOCL=True, bPlotPy=False,  bPlotOcl=True,  bOMP=False, nBlur=0, cSOR=0.0, cV=0.5, yrange=None, bPlot1D=True , bSlab=False, z_slab=None, bOld=False ):
    ...
    spff.setupEwaldGrid( ns, dg=dg, pos0=pos0 )                                   # setup grid in C++ for projection
    dens = spff.projectAtomsEwaldGrid( apos, qs, ns=ns, order=order )             # project atoms to grid using C++ 
