...
`./run_gui.sh --script demos/gui_scripts/NAME.py [-- extra args]`, or selected
...
- **conference_demo.py** — Paced generator: build 4×PTCDA → deterministic
  ...
  - Example (fast): `./run_gui.sh --script demos/gui_scripts/conference_demo.py`
  - Example (paced): `./run_gui.sh --script demos/gui_scripts/conference_demo.py --script-delay-ms 300 --script-points-per-frame 5 --script-barriers`
- **ptcda_interactive_drag.py** — Build 4×QEq-PTCDA on the NaCl FAF, select
  ...
  - Example: `./run_gui.sh --script demos/gui_scripts/ptcda_interactive_drag.py`
- **ptcda_drag_demo.py** — Paced generator: build 2×PTCDA on NaCl → relax →
  ...
  - Example: `./run_gui.sh --script demos/gui_scripts/ptcda_drag_demo.py`
...
- **folded_rigid_setup.py** — Load a molecule, fit or load a folded substrate
  ...
  - Arguments: `--mol`, `--substrate`, `--fit`, `--fit-fit`, `--run`, `--step`,
    `--manip`, `--n`, `--dt`, `--x`, `--y`, `--z`, `--k`
  - Example: `./run_gui.sh --script demos/gui_scripts/folded_rigid_setup.py -- --mol data/xyz/H2O.xyz --fit data/fits/h2o_nacl.npz --run`
...
- **azaindol_draw_offline.py** — Headless hex→pent→N→H→copy/paste/rotate dimer; one SVG per step
  - Example: `PYTHONPATH=. python demos/gui_scripts/azaindol_draw_offline.py`
- **azaindol_draw_demo.py** — Same sequence via live GUI tools; full-window PNG frames → GIF
  - Example: `./run_gui.sh --script demos/gui_scripts/azaindol_draw_demo.py`
...
- `spammm/GUI/gui_script_utils.py` — widget helpers + demo overlays / PNG+GIF+MP4 capture
  (`frames_to_gif`, `frames_to_video` — H.264 `-tune animation` for mostly-static content)
