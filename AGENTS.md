...
- **File references (HARD RULE):** when referencing files for the USER — especially produced figures/artifacts — ALWAYS give one clickable reference tag per concrete file: `<ref_file file="/absolute/path/name.png" />`. NEVER use glob/brace patterns like `foo_{S,J}.png`, `runs/*`, `*.xyz`, markdown `[x](file://...)` links, or bare paths — the user cannot click or navigate those. Expand every file individually.
- **Molecule / PBC-cell plots (SSOT):** always `plotUtils.plot_pbc_chain_cell` (or `plot_bond_scalar_map` for bond-scalar maps) — canonical style = thin grey skeleton lines (lw~1), small element-colored atom dots (`sizes=ELEMENT_DICT[e][6]*sz`, `linewidths=0`, NO rims), junctions: covalent (bonded/short) X–H side solid green + H···acceptor side magenta dashed — pick the solid side by actual distance so the bonded N–H is always unambiguous, magenta cell box. NEVER ad-hoc matplotlib for molecule/cell figures; do not re-derive style per request.
