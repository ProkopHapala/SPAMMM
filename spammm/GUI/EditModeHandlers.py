...
class UnifiedMode(EditModeHandler):
    ...
    def resolve_target(self, p_world):
...
class AtomMode(EditModeHandler):
    ...
    def on_press(self, event, p_world, ctrl):
        if event.button == 1:
            ...
            # Atom clicks handled by scene → sig_atom_clicked → on_atom_click
...
class PiMode(EditModeHandler):
    ...
    def on_press(self, event, p_world, ctrl):
        if event.button == 1:
            # Atom clicks handled by scene → sig_atom_clicked → on_atom_click (cycles pi)
            selected = self.scene.get_selected_ids()
            if selected: return
            self._add_free_atom(p_world)
        elif event.button == 2:
