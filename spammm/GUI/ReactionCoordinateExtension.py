...
def build_ui(window):
    ...
    g_hb.add(window.rc_all_hbonds_chk)
    layout.addLayout(g_hb.layout())

    g_opts = AutoGridPlacer(cols=4)
    ...
    g_opts.add(window.rc_relax_endpoints_chk)
    layout.addLayout(g_opts.layout())

    g_dx = AutoGridPlacer(cols=4)
    ...
    g_esp.add_pair("ESP z:", window.rc_esp_z_spin)
    ...
    g_esp.add_pair("grid:", window.rc_esp_n_spin)
