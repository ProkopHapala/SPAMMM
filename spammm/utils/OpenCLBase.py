"""
OpenCLBase.py — Base class for all PyOpenCL-accelerated computations in SPAMMM.

Purpose: Provide shared infrastructure for GPU-accelerated modules: OpenCL context
management, kernel loading, buffer allocation/reuse, and host-device data transfer.
Every PyOpenCL module (SPFF, UFF, GridFF, AFM, RigidBodyDynamics) inherits from this.

Key functionality:
  - Device selection by vendor (NVIDIA, AMD, Intel) or index
  - Kernel program loading from .cl files with build options and caching
  - Buffer dictionary: named GPU buffers with automatic allocation/reuse
  - toGPU_() / fromGPU_() — typed data transfer with format strings
  - set_args() — kernel argument binding with automatic buffer resolution
  - run() / run_vec() — kernel launch with grid-stride loops and 2D/3D ranges

Role in SPAMMM: The GPU foundation. All OpenCL modules inherit from OpenCLBase.
It abstracts away device selection, compilation, and buffer management so that
force field and simulation code can focus on physics, not boilerplate.
...
class OpenCLBase:
    ...
    def load_program_multi(self, kernel_paths, build_options=None, bMakeHeaders=True, bPrint=False):
        ...
        Args:
            kernel_paths (list): List of absolute paths to .cl files, concatenated in order
            build_options: OpenCL build options
            bMakeHeaders (bool): Extract kernel headers from combined source
            bPrint (bool): Print debug info
        ...
        """
        if self.prg is not None:
            print(f"load_program_multi({kernel_paths!r} - already loaded)")
            return True
        source_parts = []
        for path in kernel_paths:
            if not os.path.exists(path):
                print(f"OpenCLBase::load_program_multi() ERROR: Kernel file not found: {path}")
                return False
        ...
        try:
            ...
            print(f"OpenCLBase::load_program_multi() ERROR: Failed to build kernels: {kernel_paths}")
            print(f"Error: {str(e)}")
            raise e
        if bPrint:
            print(f"OpenCLBase::load_program_multi() Successfully loaded {len(kernel_paths)} kernel files")
        return True
