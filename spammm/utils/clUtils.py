"""
clUtils.py — OpenCL device selection and grid data structure helpers.

Purpose: Utility functions for selecting OpenCL devices by vendor, and helper
classes (GridShape, GridCL) for managing 3D grid dimensions and OpenCL grid images.

Key functionality:
  - get_nvidia_device() — find NVIDIA GPU platform
  - GridShape — 3D grid dimensions with origin and step vectors
  - GridCL — OpenCL image/grid buffer management

Role in SPAMMM: Shared OpenCL utilities. Used by OpenCLBase (device selection)
and GridFF.py (grid dimension management).
"""
