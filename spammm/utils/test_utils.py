"""
tests/utils.py — Shared test utilities for SPAMMM.

Purpose: Common helper functions used across test scripts: potential computation,
plotting, RMS error calculation, and data comparison tools.

Key functionality:
  - compute_potential() — reference potential at arbitrary points (brute force)
  - plot1Dcut() — 1D potential profile visualization
  - rms_error() — root-mean-square error between two arrays

Role in SPAMMM: Test infrastructure. Provides reference implementations and
plotting helpers for parity checks between CPU and GPU code paths.
"""
...
def plot1Dcut(apos, qs, Vg, i0, dg, Ls, iax=0, nPBC=[10,10,10], vmax=5.0, scErr=100.0, Vref=None, spff=None ):
    ...
    if Vref is None:
        fe  = spff.sampleCoulombPBC(ps, apos, qs, lvec=lvec, nPBC=nPBC)
        Vref = fe[:, 3]
    else:
