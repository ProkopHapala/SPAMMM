#!/usr/bin/python
"""
AtomicSystem.py — Array-based molecular representation for force field evaluation.

Purpose: Store molecular geometry as flat NumPy arrays (positions, enames, atom types,
bonds, angles, torsions) for efficient numerical operations. This is the "compiled"
form of a molecule — optimized for force field computation and rendering, not editing.

Key functionality:
  - File loading: from_mol, from_mol2, from_xyz, from_npz
  - Property arrays: apos, enames, atom_types, bonds, angles, torsions
  - Neighbor lists and neighbor cells for periodic boundary conditions
  - to_xyz() — export to XYZ format

Role in SPAMMM: The numerical workhorse. AtomicGraph (editor state) is synced
to AtomicSystem for rendering (VispyUtils) and force field evaluation (SPFF, UFF).
All OpenCL force field wrappers consume AtomicSystem instances.
"""
                ...
                if( 'mol' == ext ):
                    ...
                    if len(tmp) >= 8:
                        (self.apos,self.atypes,self.enames,self.qs,
                         self.bonds, self.lvec, self.atom_types, self.atom_types_spff) = tmp
                    else:
