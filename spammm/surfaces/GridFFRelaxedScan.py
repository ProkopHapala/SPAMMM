...
Uses GridFF for substrate interactions and SPFF+MD for intra-molecular relaxation.
...
and find optimal molecular orientations on surfaces. Combines GridFF + SPFF + MD.
"""
...
def prepare_molecule(mol_path, type_map=None, z_shift=0.0, find_bonds=True):
    ...
    mol.atom_types_spff = [tmap.get(e, e) for e in mol.enames]
    return mol
...
class GridFFRelaxedScan:
    ...
    def prepare(self, anchor_z_above=4.0, lateral_shift=(0.0, 0.0), generate_grid=True):
        ...
        self.mm.toSPFFsp3_loc(mol=self.mol, atom_types=self.mm.atom_types, bRealloc=True, bEPairs=False, bUFF=False)
