"""
GridFF.py — PyOpenCL B-spline grid force field for molecule-surface interactions.

Purpose: Precompute a 3D grid of Pauli repulsion, London dispersion, and Coulomb
potentials for a substrate, then interpolate at arbitrary points via B-splines for
fast force evaluation during molecular dynamics.

Key functionality:
  - GridFF_cl: OpenCL wrapper for grid construction and sampling
  - B-spline interpolation for smooth potential evaluation
  - Combined potential: Pauli + London + Coulomb (PLQd format)
  - sample3D_comb() — sample combined potential at multiple points on GPU
  - GridShape / GridCL — grid dimension and buffer management

Role in SPAMMM: Surface interaction engine. Used by MolecularDynamics.py
(surface forces), GridFFRelaxedScan.py (PES scans), and RigidBodyDynamics.py
(substrate sampling). The gridFF.cl kernel handles B-spline interpolation.
"""
...
from ..utils.clUtils import GridShape,GridCL
...
class GridFF_cl:
    ...
    def __init__(self, nloc=32 ):
        ...
        try:
            kernel_dir = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../kernels'))
            kernel_paths = [
