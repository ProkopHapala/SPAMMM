...
Key functionality:
  - read_atom_types(), read_element_types() — parse .dat files
  - SPFFparams — full SPFF parameter database (atoms, bonds, angles, torsions, vdw)
  - ElementType — single element record with RvdW, EvdW, Qbase, mass, color

Role in SPAMMM: Parameter loader. Used by UFFbuilder.py (UFF parameter assignment)
and SPFF.py (SPFF force field setup). All parameter files live in data/.
"""
...
class AtomType:
    ...
    def __init__(self, name, parent_name="*", element_name="", epair_name="", valence=0, nepair=0, npi=0, sym=0,
                 Ruff=0.0, RvdW=0.0, EvdW=0.0, Qbase=0.0, Hb=0.0, bSPFF=False, Ass=0.0, Asp=0.0, Kss=0.0, Ksp=0.0, Kep=0.0, Kpp=0.0,
        ...
        self.bSPFF  = bSPFF              # Flag for SPFF parameters
...
def generate_REQs_from_atom_types(mol, atom_types):
    ...
    Prefer MOL2 atom_types_spff (underscore form) if present; fall back to enames.
    ...
    for ia in range(natom):
        atom_name = at_spff[ia] if (at_spff is not None) else mol.enames[ia]
...
SPFFparams.assignRE = assignRE

# ---- Data path for .dat files ----
...
def make_REQs_from_enames(enames, qs, atom_types, type_map=None):
    """Build SPFF REQ array (R, sqrt(E), Q, H) from element/type names and charges.
    enames:     list of element names or atom type names
    qs:         array of charges (from xyz 4th column)
    atom_types: dict from SPparams.read_atom_types
    type_map:   optional dict mapping element name -> atom type name (e.g. {'C': 'C_R'})
    Returns: (N,4) float32 array in SPFF REQ layout"""
    N = len(enames)
...
def load_xyz_with_REQs(fname, atom_types=None, type_map=None):
    ...
    if atom_types is None:
        ...
        atom_types = read_atom_types(os.path.join(_DATA_PATH, 'AtomTypes.dat'), etypes)
