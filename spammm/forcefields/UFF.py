...
Role in SPAMMM: Alternative covalent force field to SPFFsp3. Used when UFF
parameters are preferred (e.g., for metal-organic systems or when SPFFsp3
...
class UFF_CL(OpenCLBase):
    ...
    def __init__(self, nloc=32, kernel_path=None, bPrint=False, debug_build_options=None):
        ...
        if not self.load_program_multi(kernel_paths, bPrint=bPrint, build_options=debug_build_options):
            print(f"Failed to load UFF kernels from {kernel_paths}")
            return
