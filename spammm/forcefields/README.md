...
- **RigidBodyDynamics.py** — 6-DOF rigid body GPU dynamics; FIRE/Newton plus `reset_dynamics_state` for pose/anchor discontinuities. **PairFF:** shared all-molecule buffers, FAF-capable ping-pong `run_multimol_md`, experimental persistent/single-WG variants, `attach_pairff_faf`, fused active-molecule kernels, and replica energy/clash evaluation. Host/GPU anchors initialize together with disabled `w=-1`.
