...
gometry optimization and MD trajectories. Handles multi-system simulations.

Key functionality:
  - FIRE optimization: global and local damping for fast convergence
  - Velocity Verlet: standard MD integrator with step, damping, temperature
  - Buffer management: apos, aforce, avel, fneigh, lvecs, ilvecs for multi-system
  - relax() — run FIRE until convergence or max steps
  - run() — run velocity Verlet for a given number of steps

Role in SPAMMM: The geometry optimizer. Used by GridFFRelaxedScan.py (relaxed
PES), ManipulationPathOpt.py (tip manipulation), and any module that needs
energy minimization or MD trajectories. Loads concatenated kernels (common+Forces+SPFF+nonbonded+gridFF+surface).
"""
...
class MolecularDynamics(OpenCLBase):
    ...
    for molecular dynamics simulations using concatenated modular OpenCL kernels.
    """
    ...
    def realloc(self, spff, nSystems=1 ):
        """
    ...
    def _setup_small_system_nonbond(self):
        ...
        # Download current per-system topology params from spff_list[0] and broadcast
        mm = self.spff_list[0]
        ...
        if self.enable_nonbond and (self._nb_bufs.get('excl_nb') is not None):
            if getattr(mm, 'excl', None) is None:
                raise ValueError("_setup_small_system_nonbond(): enable_nonbond=True but spff.excl is None")
    ...
    def allocate_cl_buffers(self, spff):
        """
        Allocates OpenCL buffers based on the SPFF template and number of systems.
        ...
        # SPFF.cl getSPFFf4() uses argument name 'fapos' for forces; alias it to the same buffer
        self.buffer_dict['fapos'] = self.buffer_dict['aforce']
        # SPFF.cl nonbond kernels use argument names `atoms` and `forces`
        ...
        # SPFF.cl getSPFFf4() uses argument name 'REQKs' for REQ parameters; alias it to the same buffer
        ...
        # NOTE: updateAtomsSPFFf4 indexes constr/constrK by iaa=iG+iS*natoms (only atoms have constraints)
    ...
    def _print_pack_system_params(self, iSys, spff):
        print(f"[pack_system dbg] system={iSys}")
        print(f"  apos shape={spff.apos.shape}\n{spff.apos}")
        print(f"  fapos shape={getattr(spff, 'fapos', None).shape if hasattr(spff, 'fapos') else 'N/A'}")
        print(f"  REQs shape={spff.REQs.shape}\n{spff.REQs}")
        print(f"  apars shape={spff.apars.shape}\n{spff.apars}")
        print(f"  bLs shape={spff.bLs.shape}\n{spff.bLs}")
        print(f"  bKs shape={spff.bKs.shape}\n{spff.bKs}")
        print(f"  Ksp shape={spff.Ksp.shape}\n{spff.Ksp}")
        print(f"  Kpp shape={spff.Kpp.shape}\n{spff.Kpp}")
        print(f"  neighs shape={spff.neighs.shape}\n{spff.neighs}")
        print(f"  neighCell shape={spff.neighCell.shape}\n{spff.neighCell}")
        print(f"  back_neighs shape={spff.back_neighs.shape}\n{spff.back_neighs}")
    ...
    def setup_kernels(self):
        ...
        self.kernel_args_getSPFFf4 = None
        ok, missing = can_bind_kernel("getSPFFf4")
        if ok:
            self.kernel_args_getSPFFf4 = self.generate_kernel_args("getSPFFf4")
        elif "getSPFFf4" in self.kernelheaders:
            warn_skip("getSPFFf4", missing)
        self.kernel_args_getSPFFf4_rot = None
        if "getSPFFf4_rot" in self.kernelheaders:
            ok, missing = can_bind_kernel("getSPFFf4_rot")
            if ok:
                self.kernel_args_getSPFFf4_rot = self.generate_kernel_args("getSPFFf4_rot")
            else:
                warn_skip("getSPFFf4_rot", missing)
        ...
        self.kernel_args_updateAtomsSPFFf4 = None
        ok, missing = can_bind_kernel("updateAtomsSPFFf4")
        if ok:
            self.kernel_args_updateAtomsSPFFf4 = self.generate_kernel_args("updateAtomsSPFFf4")
        elif "updateAtomsSPFFf4" in self.kernelheaders:
            warn_skip("updateAtomsSPFFf4", missing)
        # New propagator variants (optional)
        self.kernel_args_updateAtomsSPFFf4_rot = None
        if "updateAtomsSPFFf4_rot" in self.kernelheaders:
            ok, missing = can_bind_kernel("updateAtomsSPFFf4_rot")
            if ok:
                self.kernel_args_updateAtomsSPFFf4_rot = self.generate_kernel_args("updateAtomsSPFFf4_rot")
            else:
                warn_skip("updateAtomsSPFFf4_rot", missing)
        #self.kernel_args_updateAtomsSPFFf4_RATTLE = self.generate_kernel_args("updateAtomsSPFFf4_RATTLE")
        self.kernel_args_cleanForceSPFFf4 = None
        ok, missing = can_bind_kernel("cleanForceSPFFf4")
        if ok:
            self.kernel_args_cleanForceSPFFf4 = self.generate_kernel_args("cleanForceSPFFf4")
        elif "cleanForceSPFFf4" in self.kernelheaders:
            warn_skip("cleanForceSPFFf4", missing)
    ...
    def upload_all_systems(self):
        ...
        for sys_idx in range(self.nSystems):
            self.pack_system(sys_idx, self.spff_list[sys_idx])
    ...
    def run_updateAtomsSPFFf4(self):
        ...
        self.prg.updateAtomsSPFFf4(self.queue, sz, None, *self.kernel_args_updateAtomsSPFFf4)
        self.queue.finish()
    
    def run_updateAtomsSPFFf4_rot(self):
        self.prg.updateAtomsSPFFf4_rot(self.queue, self.sz_nvec, self.sz_loc, *self.kernel_args_updateAtomsSPFFf4_rot)
        self.queue.finish()
    
    def run_updateAtomsSPFFf4_RATTLE(self):
        self.prg.updateAtomsSPFFf4_RATTLE(self.queue, self.sz_nvec, self.sz_loc, *self.kernel_args_updateAtomsSPFFf4_RATTLE)
        self.queue.finish()
    
    def run_cleanForceSPFFf4(self):
        self.ensure_queue()
        # IMPORTANT: cleanForceSPFFf4 indexes `aforce` using `nvec = natoms+nnode`.
        ...
        sz = (int(self.nvecs), int(self.nSystems))
        self.prg.cleanForceSPFFf4(self.queue, sz, None, *self.kernel_args_cleanForceSPFFf4)
        # cleanForceSPFFf4 kernel only zeros sigma portion of fneigh (nnode*4 entries).
    ...
    def run_MD_step(self, do_clean=True, do_nb=False, do_spff=True, use_rot=False, force_kernel='basic'):
        """Backward-compatible wrapper for previous API (deprecated)."""
        if use_rot:
            self.run_step_rot(do_clean=do_clean, do_nb=do_nb, do_spff=do_spff, integrator=force_kernel)
        else:
            self.run_step_basic(do_clean=do_clean, do_nb=do_nb, do_spff=do_spff, integrator=force_kernel)
    ...
    def scan_1D( self, nsteps=100, d=[0.1,0.0,0.0], p0=[0.0,0.0,0.0], use_texture=False, spff=None ):
        if spff is None: spff = self.spff_list[0]
    ...
    def update_pbc_shifts(self, lvec, nPBC):
        ...
        # Note: SPFF.cl uses pbc_shifts[ipbc0+ic]
    ...
    def pack_system(self, iSys, spff):
        """Packs data from an SPFF instance into GPU buffers for a specific system index."""
        ...
        if self.bPrintPackSystem:
            self._print_pack_system_params(iSys, spff)

        # Back-neighbor indices per vector (atoms + pi) for recoil force accumulation; default to -1 if not provided.
        # IMPORTANT: updateAtomsSPFFf4 uses these as direct indices into the global `fneigh` buffer.
        ...
        if hasattr(spff, 'back_neighs') and (spff.back_neighs is not None):
            ncopy = min(spff.back_neighs.shape[0], nvecs)
            bk[:ncopy, :] = spff.back_neighs[:ncopy, :].astype(np.int32)
            if nnode > 0:
        ...
        self.toGPU('apos',      self._flat32(spff.apos),    byte_offset=offset_atoms)
        self.toGPU('aforce',    self._flat32(spff.fapos),   byte_offset=offset_atoms)
        self.toGPU('aforce_old', self._flat32(spff.fapos),  byte_offset=offset_atoms)
        self.toGPU('REQs',      self._flat32(spff.REQs),    byte_offset=offset_REQs)
        self.toGPU('neighs',    self._int32 (spff.neighs),    byte_offset=offset_neighs)
        self.toGPU('neighCell', self._int32 (spff.neighCell), byte_offset=offset_neighs)
        ...
        if self.enable_nonbond and (self.buffer_dict.get('excl') is not None):
            if getattr(spff, 'excl', None) is None:
                raise ValueError("enable_nonbond=True but spff.excl is None; build exclusions before pack_system()")
            offset_excl = iSys * natoms * 16 * np.int32().itemsize
            self.toGPU('excl', self._int32(spff.excl), byte_offset=offset_excl)
        self.toGPU('apars',     self._flat32(spff.apars),   byte_offset=offset_apars)
        self.toGPU('bLs',       self._flat32(spff.bLs),     byte_offset=offset_apars)
        self.toGPU('bKs',       self._flat32(spff.bKs),     byte_offset=offset_apars)
        self.toGPU('Ksp',       self._flat32(spff.Ksp),     byte_offset=offset_apars)
        self.toGPU('Kpp',       self._flat32(spff.Kpp),     byte_offset=offset_apars)
        
        # Upload lattice vectors if present
        if hasattr(spff, 'lvec') and spff.lvec is not None:
            lvec_padded = np.zeros((3,4), dtype=np.float32)
            lvec_padded[:,:3] = spff.lvec
            self.toGPU('lvecs', lvec_padded.flatten(), byte_offset=offset_lvec)
            
            # Inverse lattice vectors
            try:
                inv_lvec = np.linalg.inv(spff.lvec)
            ...
            # PBC Shifts
            if hasattr(spff, 'nPBC') and spff.nPBC is not None:
                shifts = self.update_pbc_shifts(spff.lvec, spff.nPBC)
        ...
        #print("pack_system() iSys=%d" % iSys, "atoms.nbytes=%d" % spff.apos.nbytes, "REQs.nbytes=%d" % spff.REQs.nbytes, "neighs.nbytes=%d" % spff.neighs.nbytes, "apars.nbytes=%d" % spff.apars.nbytes)
        ...
        self.toGPU('MDparams',  np.array([spff.dt, spff.damp, spff.damp], dtype=np.float32), byte_offset=iSys*float4_size)




    def scan_2D( self, ns=(50,50), du=[0.1,0.0,0.0], dv=[0.0,0.1,0.0],  p0=[0.0,0.0,0.0], use_texture=False, spff=None ):
        if spff is None: spff = self.spff_list[0]
    ...
    def scanSurfMorse_2D(self, ns=(50,50), du=[0.1,0.0,0.0], dv=[0.0,0.1,0.0], p0=[0.0,0.0,0.0], spff=None):
        if self.kernel_args_getSurfMorse is None:
            raise ValueError('scanSurfMorse_2D() requires set_surface() and getSurfMorse kernel setup first')
        if spff is None:
            spff = self.spff_list[0]
        pos = np.zeros((ns[0], ns[1], 4), dtype=np.float32)
        forces = np.zeros((ns[0], ns[1], 4), dtype=np.float32)
        du = np.array(du, dtype=np.float32)
        dv = np.array(dv, dtype=np.float32)
