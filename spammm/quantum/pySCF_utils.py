...
Purpose: Run pySCF quantum chemistry calculations (RHF, DFT) to obtain electron
densities for the FDBM AFM method. Provides grid-based density evaluation compatible
with SPAMMM's grid projection pipeline.

Key functionality:
  - run_pyscf_scf() — run SCF and return density matrix
  - get_density_on_grid() — evaluate electron density on a 3D grid
  - Supports RHF, UHF, and DFT with various functionals

Role in SPAMMM: Ab initio backend for FDBM. Used by ModularPipeline.py (pySCF
backend) and AFM_utils.py (density providers). Alternative to DFTB when higher
accuracy or different basis sets are needed.
"""
