#!/usr/bin/env python3
"""
DFTBplusParser.py — Parse DFTB+ basis set and wavefunction files.

Purpose: Parse DFTB+ HSD input files (wfc.*.hsd) to extract Slater-type orbital
parameters (exponents, contraction coefficients, angular momenta) for GPU density
projection. Also parses band.out and eigenvec.bin for eigenvalue/eigenvector data.

Key functionality:
  - parse_wfc_hsd() — parse wavefunction coefficients from HSD format
  - convert_wfc_to_species_list_ang() — convert to per-species angular momentum list
  - parse_band_out() — extract eigenvalues from band structure output
  - parse_eigenvec_bin() — read eigenvector binary format

Role in SPAMMM: DFTB basis parser. Used by ModularPipeline.py and Grid_dftb.py
to set up the GPU projector with correct STO basis parameters per element.
