"""
DFTBcore.py — ctypes interface to DFTB+ for SCF and matrix export.

Purpose: Provide a Python wrapper around the DFTB+ C-API for running SCF calculations
and exporting density matrices, Hamiltonian, overlap matrices, and eigenvectors.

Key functionality:
  - init() — initialize DFTB+ with an HSD input file
  - run_scf() — self-consistent field calculation
  - get_dm_dense() — export density matrix
  - get_eigvecs_dense() — export eigenvectors and eigenvalues
  - enable_matrix_collection() — toggle matrix export during SCF

Role in SPAMMM: DFTB+ SCF backend for FDBM/STM pipeline. Used by ModularPipeline.py
Stage 1 to obtain the density matrix and orbitals for GPU projection in Stage 2.

Pattern follows spammm/Fireball/FireCore.py and spammm/WavePlot/WavePlot.py

Usage:
    from spammm.DFTBcore import DFTBcore
...
_POSSIBLE_PATHS = [
    # From spammm/DFTBcore.py -> ../../../_build/app/dftbcore/libdftbcore.so
